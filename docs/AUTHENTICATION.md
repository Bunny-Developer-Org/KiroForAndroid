...
- **The QR carries the address and the pairing code, and nothing else.** No device token, no API key. A QR gets photographed, screenshotted and pasted into chat windows; what it holds must be survivable, which is exactly what single-use and ~5 minutes buy. `PairingPayload` in `core/` is the only definition of that payload, so the bridge and the app cannot drift on it.
- **A loopback check is not a loopback check behind a tunnel.** `cloudflared` connects to `http://127.0.0.1:8765`, so every tunnelled request arrives with `remoteHost == "127.0.0.1"` (`BridgeServer.kt`). Anything that must be reachable only from the bridge host is a filesystem socket in a `0700` directory — never an HTTP route behind an origin check, which the whole public internet would satisfy. This is why `kiro-bridge pair` talks to a Unix socket.

- **`/qr` is the only Access-gated route, and it verifies the assertion itself.** `/`, `/pair` and `/acp` stay open to a caller with no browser session, because the Android app cannot complete an interactive Google sign-in — an Access application scoped to the whole hostname breaks pairing *and* the session socket, with an error pointing nowhere near Cloudflare. And the `Cf-Access-*` headers are not evidence: Cloudflare strips inbound copies on requests it proxies, but the origin port is reachable without going through the tunnel at all, so anything that reaches it can assert any identity. `AccessVerifier` therefore checks the RS256 signature against `https://<team>/cdn-cgi/access/certs` plus `alg`, `exp`, `nbf`, `iss` and `aud`, and the email the page displays comes out of the *verified token*, never out of `Cf-Access-Authenticated-User-Email`.
- **Rotation bounds what a *seen* code is worth, and nothing else.** `/qr` mints a new code every 30s; the one it replaces keeps working for 30s more so that a scan landing just after the picture changes still pairs. That shrinks the window in which a code someone photographed, screen-shared or read over your shoulder can be redeemed ahead of your phone. It does nothing about a compromised laptop or a malicious browser extension, both of which see every code the page ever shows. The rule that replaces "exactly one pending code" is: *every code that will redeem is one a human is looking at right now, or one that was on a screen within the last 30 seconds* — two per surface in steady state, and not a brute-force change against 32⁸ at five attempts a minute in any case.

  The same flattening has a **known, unfixed consequence**: `PairingService`'s per-address rate limiter sees one address for every tunnelled client, so on a tunnelled bridge it is a single global bucket of 5 attempts per 60 seconds, checked before the code is examined. Brute force is not the worry (32⁸ ≈ 1.1×10¹² codes); availability is — five junk requests from any internet scanner make a legitimate code return `429` until the window clears. Tracked as its own backlog item rather than fixed here, and the fix must not naively trust `X-Forwarded-For`.

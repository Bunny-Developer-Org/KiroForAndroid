...
Because sessions live account-side, the app pairs with **a set** of bridges and may attach a session through any of them. The app stores bridges as a list and tracks last-seen per bridge (`DataStoreBridgeRegistry`). Switching bridges for a session is legal and costs a transcript refetch, and is a deliberate user action via `BridgeListScreen`'s add/remove/switch UI (F-07, shipped 2026-09-02) — not yet automatic. The design intent is still to prefer the most recently reachable bridge, but that preference is not implemented: as of 2026-09-02 the active bridge is whichever the user last selected (or the first in the stored list), with no sort by `lastSeenMillis`. The reconnect loop itself (backoff, connectivity-regained retry) currently lives in `MainActivity`'s `reconnectLoop()`, not in `SessionConnectionService` — that class is presently only the Android foreground-service/notification wrapper (F-15). Consider this section's ownership claim aspirational until F-15's follow-up work moves selection logic into the service, or until this ADR is corrected to describe `MainActivity` as the owner. The UI shows which bridge is in use only when there is more than one.
